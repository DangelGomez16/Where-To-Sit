...
int main()
{
    ...
    int tracker = 0;
    int orders = permute(rowArr, pairArr, numPeop, tracker, popCorn);
    printf("%d\n", orders);
...
int permute(int *perm, int **pair, int numPeop, int tracker, int *popCorn)
{
    while(1)
    {
        ...
        // If the permutation we already iterated thought to the last permutation, return
        // tracker. Otherwise, call another recursion.
        if(perm[0] == -1)
            return tracker;
